...
class Stop {
 public:
  ...
  Stop(const string& id, const string& code, const string& name,
       ...
       Stop* parent_station, const string& stop_tz,
  ...
  const std::string& getStopTimezone() const { return _stop_tz; }
 ...
 private:
  string _id, _code, _name, _desc, _zone_id, _stop_url, _stop_tz,
      _platform_code;
